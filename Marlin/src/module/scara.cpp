...
float segments_per_second = TERN(AXEL_TPARA, TPARA_SEGMENTS_PER_SECOND, SCARA_SEGMENTS_PER_SECOND);
  ...
  void scara_set_axis_is_at_home(const AxisEnum axis) {
    ...
    const float x = raw.x - scara_offset.x, y = raw.y - scara_offset.y,
                hand = SCARA_IS_RIGHT_HANDED ? 1 : -1,
                THETA3 = ATAN2(y, x), c2 = HYPOT2(x, y), rc = RSQRT(c2),
                THETA1 = THETA3 - hand * ACOS((c2 + sq(L1) - sq(L2)) * rc * RECIPROCAL(2.0f * L1)),
                THETA2 = THETA3 + hand * ACOS((c2 + sq(L2) - sq(L1)) * rc * RECIPROCAL(2.0f * L2));
    ...
    delta.set(DEGREES(THETA1), DEGREES(THETA2 - THETA1 * SCARA_CROSSTALK_FACTOR), raw.z);
